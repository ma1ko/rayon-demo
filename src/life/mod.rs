...
impl Board {
    ...
    pub fn par_adapt_next_generation(&self) -> Board {
        use rayon_try_fold::prelude::*;
        let new_brd =
            rayon_try_fold::prelude::IntoParallelIterator::into_par_iter(0u64..self.len() as u64)
                .adaptive()
                .map(|cell| self.successor_cell(cell as usize))
                .rayon(4)
                .collect();

        self.next_board(new_brd)
    }
    pub fn par_my_adapt_next_generation(&self) -> Board {
        use rayon_try_fold::prelude::*;
        let iter =
            rayon_try_fold::prelude::IntoParallelIterator::into_par_iter(0u64..self.len() as u64)
                .adaptive();
        let new_brd = adaptive_iterator::adaptive::mk_adaptive(iter)
            .map(|cell| self.successor_cell(cell as usize))
            .rayon(4)
            .collect();
...
fn par_adapt_generations(board: Board, gens: usize) {
    let pool = rayon::ThreadPoolBuilder::new().build().unwrap();
    pool.install(|| {
        let mut brd = board;
        for _ in 0..gens {
            brd = brd.par_adapt_next_generation();
        }
    });
}
fn par_my_adapt_generations(board: Board, gens: usize) {
    let pool = rayon::ThreadPoolBuilder::new().build().unwrap();
    pool.install(|| {
        let mut brd = board;
        for _ in 0..gens {
            brd = brd.par_my_adapt_next_generation();
        }
    });
}
...
fn parallel_adapt_generations_limited(board: Board, gens: usize, min_interval: Duration) {
    let mut brd = board;
    let mut time = Instant::now();
    for _ in 0..gens {
        brd = brd.par_adapt_next_generation();
...
pub fn main(args: &[String]) {
    ...
    if args.cmd_bench {
        ...
        // );
        let par_adapt = measure(par_adapt_generations, &args).as_nanos();
        println!(
            "par_adapt   : {:10} ns -> {:.2}x speedup",
            par_adapt,
            serial as f64 / par_adapt as f64
        );
        let par_my_adapt = measure(par_my_adapt_generations, &args).as_nanos();
        println!(
            "par_my adapt: {:10} ns -> {:.2}x speedup",
            par_my_adapt,
            serial as f64 / par_my_adapt as f64
        );
    }
    ...
    if args.cmd_play {
        ...
        let par_adapt = measure_cpu(parallel_adapt_generations_limited, &args);
        println!("par_adapt: {:.2} fps", par_adapt.actual_fps);
        if let Some(cpu_usage) = par_adapt.cpu_usage_percent {
            println!("  cpu usage: {:.1}%", cpu_usage);
        }
    }
}
